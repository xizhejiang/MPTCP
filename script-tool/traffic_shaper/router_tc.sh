...
echo "wlan0 traffic"
tc class add dev $DEV parent 1:1 classid 1:11 htb rate 500kbps ceil 600kbps
tc filter add dev $DEV parent 1:0 protocol ip prio 1 u32 match ip dst $WLAN0 flowid 1:11

echo "wlan1 traffic"
tc class add dev $DEV parent 1:1 classid 1:12 htb rate 400kbps ceil 500bps
tc filter add dev $DEV parent 1:0 protocol ip prio 1 u32 match ip dst $WLAN1 flowid 1:12

echo "other devices traffic"
tc class add dev $DEV parent 1:1 classid 1:10 htb rate 175kbps ceil 475kbps
